...
const getStories = async (location = 1) => {
  try {
    ...
    console.log('Stories loaded:', data.listStory.length);
...
// ===== NOTIFICATION FUNCTIONS (MOVED UP BEFORE renderMainPage) =====
    ...
    if (subscription) {
      ...
      const response = await fetch('https://story-api.dicoding.dev/v1/notifications/subscribe', {
        ...
        headers: {
          ...
          'Authorization': `Bearer ${localStorage.getItem('authToken')}` // pastikan token tersimpan di localStorage
        },
      ...
      if (!response.ok) {
        ...
        console.error('❌ Gagal mendaftarkan notifikasi:', errText);
        showNotification('Gagal mendaftarkan notifikasi ke server', 'error');
      } else {
        console.log('✅ Subscription berhasil didaftarkan ke server');
      }
...
const renderMainPage = () => {
  ...
  // Load notification status after DOM is ready
  setTimeout(() => {
...
let selectedLocation = { lat: -6.200000, lon: 106.816666 };
